...
def transpose_rechunk(ds,
    ...
    if subsampling is not None:
        ...
        ds = ds.isel(**{i_dim: slice(0,None,subsampling),
...
def print_rechunk(r, v):
    ...
    print('Source chunk size: \t\t '
          +'x'.join('{}'.format(v) for v in s.data.chunksize)
          +'\t\t {:.1f}MB'
          .format(np.prod(s.data.chunksize)*4/1e6)
         )

    print('Source number of files: \t\t {:d}'
          .format(int(np.prod(s.shape)/np.prod(s.data.chunksize)))
         )

    #
    print('Intermediate chunk size: \t '
          +'x'.join('{}'.format(v) for v in i.chunks)
          +'\t\t {:.1f}MB'.format(np.prod(i.chunks)*4/1e6)
         )
    print('Intermediate number of files: \t\t {:d}'
          .format(int(np.prod(s.shape)/np.prod(i.chunks)))
         )

    #
    print('Target chunk size: \t\t '
          +'x'.join('{}'.format(v) for v in t.chunks)
          +' \t\t {:.1f}MB'.format(np.prod(t.chunks)*4/1e6)
         )
    print('Target number of files: \t\t {:d}'
          .format(int(np.prod(s.shape)/np.prod(t.chunks)))
         )
